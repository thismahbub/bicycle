    ...
    <body bgcolor="#ff6699">
        ...
        <table border="0" cellpadding="5" cellspacing="5" width="900" bgcolor="#FFFFFF" id="container">
            ...
            <td width="750" valign="top"><div id="r">Profile Name: <%=login.getMemberName()%>&nbsp;&nbsp;Member Email: &nbsp;<%=login.getEmail()%>&nbsp;<a href="logoutProcess.jsp">logout</a></div><div>
                    <div style="color:#090; font-weight:bold;font-size:16px;"><u>Member Information</u></div><hr>
                    ...
                    <%
                                ...
                                Vector<Login> searchResult = (Vector<Login>) myEntityManager.getEntityManager().createNamedQuery("Login.findAll").getResultList();

                                for (int i = 0; i < searchResult.size(); i++) {
                    %>

                            <td> <%= searchResult.get(i).getUsername()%></td>
                            <td> <%= searchResult.get(i).getPassword()%></td>
                            <td> <%= searchResult.get(i).getMemberName()%></td>
                            <td> <%= searchResult.get(i).getEmail()%></td>
                            <td> <%= searchResult.get(i).getLevel()%></td>
                            <td width="150"><a href="admin_memberInfo_edit.jsp?userName=<%= searchResult.get(i).getUsername()%>">Edit</a></td>
                            <td width="150"><a href="admin_bicycleDetail_Delete.jsp?userName=<%= searchResult.get(i).getUsername()%>">Delete</a></td>
                        </tr>
                        <%
                                    }
                        %>
